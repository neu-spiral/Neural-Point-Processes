...
__all__ = ["CNPPFLoss", "CNPFLoss", "ELBOLossLNPF", "SUMOLossLNPF", "NLLLossLNPF"]

def masked_sum_log_prob(prob, mask, sample):
    """Compute log probability then sum all but the z_samples and batch."""
    # size = [n_z_samples, batch_size, *]
    log_p = prob.log_prob(sample)   # Apply mask to log probabilities
    log_p = log_p * mask
    # size = [n_z_samples, batch_size]
    sum_log_p = sum_from_nth_dim(log_p, 2)  # Sum along the target dimension (last axis)
    return sum_log_p
...
class CNPPFLoss(BaseLossNPF):
    """Loss for conditional neural process (sub-)family on masked pixels."""

    def forward(self, pred_outputs, X_trgt, Y_trgt):
        p_yCc, _, _, _ = pred_outputs  # Extract p_yCc from the model's output
        return self.get_loss(p_yCc, _, _, _, X_trgt, Y_trgt)

    def get_loss(self, p_yCc, _, q_zCc, ___, X_trgt, Y_trgt):
        assert q_zCc is None
        
        # Create a binary mask from X_trgt (1 for observed, 0 for unobserved)
        mask = X_trgt.squeeze().unsqueeze(0)  # Shape: [batch_size, height, width, 1]
        
        # Compute log probability for observed pixels only
        sum_log_p_yCz = masked_sum_log_prob(p_yCc, mask, Y_trgt)
        # print(sum_log_p_yCz.shape)
        # Calculate the NLL loss (negative log likelihood)
        nll = -sum_log_p_yCz.squeeze(0)  # Size = [batch_size]
        # print(nll.shape)
        return nll.mean()
